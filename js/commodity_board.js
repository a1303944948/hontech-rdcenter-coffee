...
function startbodyS(){
	...
	ajax({
		...
		success: function(data){
			console.log(data);
			var commodityName = c('commodity_name')[0];
			var commodityNameArr = [];
			for(var i = 0; i < data.data.length; i++){
				var commodityNameObj = {};
				commodityNameObj.name = data.data[i].productName;
				commodityNameObj.value = data.data[i].code;
				commodityNameArr.push(commodityNameObj);
			}
			commodityName.setAttribute('data-select',JSON.stringify(commodityNameArr));
			WmStartSelect();
		}
	})
}
	...
	for(var i = 0; i < BodyLeftList.length; i++){
		...
		(function(q){
			BodyLeftList[q].onclick = function(){
				...
				detailedOperatorCompanyname.setAttribute('data-value',commmodityLeft[q].ingredientCode);
...
function submit(){
	...
	bodyCreat.onclick = function(){
		...
		d('commodity_name').setAttribute('data-value','');					//产品ID
	...
	bodySubmit.onclick = function(){
		...
		var commodityId = d('commodity_name').dataset.value;			//产品ID
		...
		$.ajax({
			...
			success: function(data){
				...
				commodityobj.ingredientCode = commodityId;
