...
//添加事件触发器
HTMLElement.prototype.Add = function(even,method){	//dom为要绑定事件的元素 even为事件名称 emthod为传入一个方法function(){}
...
//PC网页端实现拖动元素方法封装
HTMLElement.prototype.Mouse = function(method,methods){	//method为鼠标点击元素时需要触发的方法 methods为开始拖动元素时触发的方法
	var that = this;
	that.onmousedown = function(e){
		var obj = method(that,e);
		document.onmousemove = function(even){
			methods(that,even,obj);
		};
	...
	xhr.onreadystatechange = function(){
		if(xhr.readyState === 4){
			if(xhr.status === 200){
				...
				obj.error?obj.error(xhr.status):console.log('用户未添加error方法');
			}
		}
	}
}
...
//js中批量修改样式的方法封装
HTMLElement.prototype.setStyle = function(json){	//dom为元素 json为要更改的样式键值对
	for(var i in json){
...
//js中批量修改多个元素样式的方法封装
function setStyles(domArr,json){	//dom为元素 json为要更改的样式键值对
	for(var i = 0; i < domArr.length; i++){
		for(var j in json){
			if(!json.hasOwnProperty(j)) continue;
			domArr[i].style[j] = json[j];
		}
	}
}
